...
//! modal chrome + the palette's own `.palette` CSS), a search input row showing
//! the live `query`, a fuzzy-filtered list of representative commands with
...
// Modal geometry constants (ui-spec §6.4 `.modal` + lightjj's `.palette` CSS).
const MODAL_W: f64 = 520.0; // ui-spec §6.4 `.modal` width (the palette uses 580; spec value chosen)
const MODAL_TOP_FRAC: f64 = 0.20; // `top: 20%`
const MODAL_RADIUS: f64 = 8.0; // ui-spec §6.4 radius 8 (palette CSS uses 14; spec value chosen)
const PAD_X: f64 = 16.0; // horizontal padding inside the modal
const INPUT_H: f64 = 42.0; // `.palette-input-wrap` padding 12 + line ~18
const ROW_H: f64 = 34.0; // `.palette-item` padding 8 + line ~18
const FOOTER_H: f64 = 34.0; // `.key-footer` padding 8 + line
...
pub fn render(scene: &mut Scene, viewport: Rect, query: &str, ctx: &RenderCtx) {
    ...
    // Height = input row + (1 row per match, or one "empty" row) + footer.
    let list_h = if matches.is_empty() {
        ROW_H
    } else {
        ROW_H * matches.len() as f64
    };
    let modal_h = INPUT_H + list_h + FOOTER_H;
    ...
    // (e) Key-footer hint bar.
    let footer = Rect::new(modal.x0, modal.y1 - FOOTER_H, modal.x1, modal.y1);
    draw_footer(scene, footer, ctx);
}

/// (b) The modal box: base bg, 1px surface1 border, rounded. The heavy
/// drop-shadow (`--shadow-heavy`) is approximated — Vello blur is expensive —
/// by a crisp surface2 ring just outside the border, reading as a thin lip.
fn draw_modal_chrome(scene: &mut Scene, modal: Rect, t: &Palette) {
    // Subtle shadow lip just outside the border (cheap shadow approximation).
    let lip = modal.inflate(1.0, 1.0);
    stroke_round(scene, lip, MODAL_RADIUS + 1.0, t.surface2.multiply_alpha(0.35), 1.0);
...
fn draw_command_row(scene: &mut Scene, row: Rect, cmd: &Command, active: bool, ctx: &RenderCtx) {
    ...
    if active {
        // `.palette-item-active` → `--bg-selected` (amber @ 8%) with the 2px
        // amber inset bar lightjj uses for the active/selected row.
...
/// (e) Footer hint bar (`.key-footer`): mantle bg, top border `--surface0`, a
/// row of "kbd label" pairs explaining navigation.
fn draw_footer(scene: &mut Scene, footer: Rect, ctx: &RenderCtx) {
    let t = ctx.theme;
    fill_rect(scene, footer, t.mantle);
    fill_rect(scene, Rect::new(footer.x0, footer.y0, footer.x1, footer.y0 + 1.0), t.surface0);

    let cy = footer.center().y;
    let mut x = footer.x0 + PAD_X;
    // Each hint = a kbd chip + a dimmed label, separated by a 14px gap.
    for (key, label) in [("\u{2191}\u{2193}", "navigate"), ("\u{21b5}", "run"), ("esc", "close")] {
        let after = draw_kbd_left(scene, x, cy, key, ctx);
        let base = baseline_for(cy, font::FS_SM, &ctx.fonts.ui);
        x = text::draw_text(scene, &ctx.fonts.ui, font::FS_SM, t.subtext0, after + 5.0, base, label);
        x += 14.0; // `gap: 14px`
    }
}
...
/// Draw a kbd chip whose LEFT edge sits at `left_x`; returns the chip's right x
/// (for chaining a trailing label). Used by the footer hint pairs.
fn draw_kbd_left(scene: &mut Scene, left_x: f64, cy: f64, label: &str, ctx: &RenderCtx) -> f64 {
    let t = ctx.theme;
    let tw = text::measure(&ctx.fonts.mono, font::FS_SM, label) as f64;
    let pad = 5.0;
    let chip_w = tw + pad * 2.0;
    let chip_h = font::FS_SM as f64 + 6.0;
    let chip = Rect::new(left_x, cy - chip_h / 2.0, left_x + chip_w, cy + chip_h / 2.0);
